...
int main(void)
{
  ...
  sum2 += sum1;
  if(sum2%10==0) printf("VALID...\n");
  else return printf("INVALID\n");
  
  
  //Credit Card Company Testing:
  while(ccNumber!=0){
      if(ccNumber>50 && ccNumber<56 && count == 14) return printf("Type: MASTERCARD\n");
      if(ccNumber==4 && (count==15 || count==12)) return printf("Type: VISA\n");
      if((ccNumber==34 || ccNumber==37) && count==13) return printf("Type: AMERICAN EXPRESS\n");
